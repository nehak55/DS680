...
dodgers <- read.csv("/Users/neha/Rpubs_doc/MDS-CH8/MDS_Chapter_8/dodgers.csv")
...
my.model <- {attend ~ ordered_month + ordered_day_of_week + cap + shirt + fireworks}
...
group.labels <- c("No Promotion","Promotion")
group.symbols <- c(21,24)
group.colors <- c("black","black") 
group.fill <- c("black","red")  
...
dodgers.train$sqrt1 <- sqrt(dodgers.train$attend)
my.model1 <- {sqrt(attend) ~ ordered_month + ordered_day_of_week + bobblehead}
train.model.fit1 <- lm(my.model1, data = dodgers.train)
plot(my.model.fit1)
summary(train.model.fit1)


my.model2 <- {log10(attend) ~ ordered_month + ordered_day_of_week + bobblehead}
train.model.fit2 <- lm(my.model2, data = dodgers.train)
plot(train.model.fit2)
summary(train.model.fit2)


my.model3 <- {(attend**2) ~ ordered_month + ordered_day_of_week + bobblehead}
train.model.fit3 <- lm(my.model3, data = dodgers.train)
plot(train.model.fit3)
summary(train.model.fit3)
