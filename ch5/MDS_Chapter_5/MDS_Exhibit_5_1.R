...
att <- read.csv("/Users/neha/Documents/Github/DS680/ch5/MDS_Chapter_5/att.csv", stringsAsFactors = FALSE)
print(str(att))
...
# --------------------------------------------
train_svm_roc <- performance(train_svm_auc, "tpr", "fpr")
test_svm_roc <- performance(test_svm_auc, "tpr", "fpr")
pdf(file = "thejointsol.pdf", 
    width = 8.5, height = 8.5)
...
# ----------------------------------------
set.seed (9999)  # for reproducibility
train_rf_fit <- randomForest(att_spec, data = train, method="class", na.action=na.omit) 
